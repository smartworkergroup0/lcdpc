-- Restore legacy user personal columns and compatibility triggers.

ALTER TABLE users
    ADD COLUMN IF NOT EXISTS name VARCHAR(200),
    ADD COLUMN IF NOT EXISTS identity_document VARCHAR(40),
...
UPDATE users u
SET name = COALESCE(per.name, u.email),
    identity_document = per.identity_document,
    tax_id = per.tax_id,
    whatsapp_phone = per.whatsapp_phone,
    full_address = per.full_address
FROM persons per
WHERE per.id = u.person_id;
